...
async def _warmup_rembg():
    # Preload the background-removal model off the request path so the first
    # /api/image/remove-bg call (and Sign PDF image tab) never hangs on a cold start.
    import threading
    try:
        from image_tools import warmup_rembg
        threading.Thread(target=warmup_rembg, daemon=True).start()
    except Exception as e:
        logging.getLogger(__name__).warning("rembg warmup not started: %s", e)
