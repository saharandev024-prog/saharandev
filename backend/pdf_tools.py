...
async def _save_upload(upload: UploadFile, job: Path) -> Path:
    # Stream the upload into an ephemeral temp file (tempfile API), then move it
    # to a sanitized, original-named path inside the request-scoped job dir so
    # downstream converters produce sensibly-named outputs. These files are only
    # needed transiently by local CLI converters (LibreOffice/Ghostscript/etc.)
    # within a single request and are removed via _cleanup() afterwards — never
    # persisted or shared across requests.
    orig = Path(upload.filename or "input")
    safe_stem = re.sub(r"[^A-Za-z0-9._-]", "_", orig.stem) or "input"
    suffix = orig.suffix or ".bin"
    fd, tmp_path = tempfile.mkstemp(dir=str(job), suffix=suffix)
    with os.fdopen(fd, "wb") as f:
        while chunk := await upload.read(1024 * 1024):
            f.write(chunk)
    await upload.seek(0)
    dest = job / (safe_stem + suffix)
