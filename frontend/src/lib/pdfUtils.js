...
// Images -> single PDF
// Photos straight from a camera/phone are huge (multi-MB each); embedding the
// original bytes makes an enormous PDF. We re-encode each image as a JPEG at
// the chosen preset (dimension cap + quality) and only fall back to the
// original bytes when the re-encoded version isn't smaller — so the PDF stays
// small while the visual quality stays high.
const IMG_PDF_PRESETS = {
  high: { maxDim: 3000, q: 0.9 },
  balanced: { maxDim: 2480, q: 0.85 },
  small: { maxDim: 1600, q: 0.72 },
};

const optimiseImageForPdf = async (file, { maxDim, q }) => {
  const url = URL.createObjectURL(file);
  try {
    const el = await new Promise((res, rej) => { const i = new Image(); i.onload = () => res(i); i.onerror = rej; i.src = url; });
    const scale = Math.min(1, maxDim / Math.max(el.naturalWidth, el.naturalHeight));
    const cw = Math.max(1, Math.round(el.naturalWidth * scale));
    const ch = Math.max(1, Math.round(el.naturalHeight * scale));
    const canvas = document.createElement('canvas');
...
export const imagesToPdf = async (files, { fit = 'fit', margin = 24, preset = 'balanced' } = {}) => {
  const out = await PDFDocument.create();
  const cfg = IMG_PDF_PRESETS[preset] || IMG_PDF_PRESETS.balanced;
  for (const f of files) {
    let bytes = new Uint8Array(await readFile(f));
    let isPng = /png$/i.test(f.type) || /\.png$/i.test(f.name);
    const opt = await optimiseImageForPdf(f, cfg);
    if (opt && opt.length < bytes.length) { bytes = opt; isPng = false; }
    let img;
