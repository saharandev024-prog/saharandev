...
const ToolPage = () => {
  ...
  const [opts, setOpts] = useState({ ranges: '1', position: 'bottom-center', start: 1, wtext: 'CONFIDENTIAL', opacity: 0.25, angle: 90, quality: 2, targetVal: 200, targetUnit: 'KB', password: '', lang: 'eng', margin: 5, url: '', html: '', imgQuality: 'balanced' });
  ...
  const DEFAULT_OPTS = { ranges: '1', position: 'bottom-center', start: 1, wtext: 'CONFIDENTIAL', opacity: 0.25, angle: 90, quality: 2, targetVal: 200, targetUnit: 'KB', password: '', lang: 'eng', margin: 5, url: '', html: '', imgQuality: 'balanced' };
  ...
  const process = async () => {
    ...
    try {
      ...
      switch (tool.engine) {
        ...
        case 'jpg-to-pdf': {
          const orig = files.reduce((s, f) => s + (f.size || 0), 0);
          const bytes = await pdf.imagesToPdf(files, { preset: opts.imgQuality });
          setResult({ name: 'images.pdf', bytes, meta: { orig, size: bytes.length } });
          break;
        }
        case 'split': {
          ...
          <div className="space-y-6">
            {files.length === 0 ? (
              ...
              <div className="rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/[0.03] p-5 sm:p-7 space-y-5">
                ...
                {tool.engine === 'jpg-to-pdf' && (
                  <Field label="PDF size & quality">
                    <div className="flex gap-2 flex-wrap">
                      {[{ id: 'high', l: 'High quality' }, { id: 'balanced', l: 'Balanced (recommended)' }, { id: 'small', l: 'Smallest size' }].map((q) => (
                        <button key={q.id} data-testid={`pdf-size-${q.id}`} onClick={() => setOpts({ ...opts, imgQuality: q.id })}
                          className={`px-4 py-2.5 rounded-xl text-sm font-semibold border transition-colors ${opts.imgQuality === q.id ? 'btn-primary text-white border-transparent' : 'border-slate-200 dark:border-white/10 hover:bg-slate-100 dark:hover:bg-white/5'}`}>{q.l}</button>
                      ))}
                    </div>
                    <p className="hint">Images are optimised (resized &amp; re-encoded) before going into the PDF, so the file stays small without looking blurry.</p>
                  </Field>
                )}
